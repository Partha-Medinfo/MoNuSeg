% Author - Partha Protim Hazarika
% The below 11 lines of code will traverse the directory structure provided
% to this function named "A" and will pick up each file with extension
% ".tif" and eventually call the function "he_to_binary_mask_final" with
% the file name as an argument one by one.

function A = loop_dir(directory)
pat = directory;
...
for k=1:numel(d)
    ...
    match=(".tif");
    newFilename = erase(filename,match);
    disp(newFilename)
    he_to_binary_mask_final(newFilename)
end
...
% Author - Partha Protim Hazarika
% The below 4 lines of code will save both binary and color masks to the drive 

s1 = strcat(filename,'-Mask.tif');
imwrite(binary_mask,s1)
s2 = strcat(filename,'-C-Mask.tif');
imwrite(color_mask,s2)